...
void __time_critical_func(pio_spi_write8_blocking)(const pio_sdio_inst_t *sdio, const uint8_t *src, size_t len) {
    size_t tx_remain = len;
    // Do 8 bit accesses on FIFO, so that write data is byte-replicated. This
    // gets us the left-justification for free (for MSB-first shift-out)
    io_rw_8 *txfifo = (io_rw_8 *) &sdio->pio->txf[sdio->sm];
    while (tx_remain) {
        if (tx_remain && !pio_sm_is_tx_fifo_full(sdio->pio, sdio->sm)) {
            *txfifo = *src++;
            --tx_remain;
        }
    }
...
void __time_critical_func(pio_spi_read8_blocking)(const pio_sdio_inst_t *spi, uint8_t *dst, size_t len) {
...
void __time_critical_func(pio_spi_write8_read8_blocking)(const pio_sdio_inst_t *spi, uint8_t *src, uint8_t *dst,
                                                         size_t len) {
    size_t tx_remain = len, rx_remain = len;
    io_rw_8 *txfifo = (io_rw_8 *) &spi->pio->txf[spi->sm];
    io_rw_8 *rxfifo = (io_rw_8 *) &spi->pio->rxf[spi->sm];
